...
export function buildArticulatedPartsPrompt(options: { description?: string; reference?: boolean; extra?: string; rows?: number; cols?: number }): string {
  ...
  const layout = standardHumanoid
    ? "Output exactly 12 isolated pieces in a strict 4 columns by 3 rows layout, ordered left-to-right and top-to-bottom: row 1 = head, torso, pelvis, separate weapon; row 2 = left upper arm, left forearm, right upper arm, right forearm; row 3 = left thigh, left shin, right thigh, right shin."
    : `Output exactly ${count} isolated pieces in a strict ${cols} columns by ${rows} rows layout, ordered left-to-right and top-to-bottom. Choose ${count} practical, non-overlapping components suitable for skeletal animation. Obey any component list in Extra requirements; otherwise split at real joints, keep left and right sides separate, and include character-specific weapons or accessories only when cells remain.`;
  const separation = `Partition the canvas into exactly ${cols} × ${rows} equal-sized cells on a perfectly regular grid, but do not draw grid lines. Use one global scale factor for the entire sheet so every piece preserves its size relative to every other piece; never scale cells or parts independently. Place the opaque bounding-box center of each piece exactly on its cell center. Keep balanced empty margin on opposite sides and at least 10% of the cell width and height clear between every visible pixel and the nearest cell boundary. Cell dimensions, center spacing, row pitch, column pitch, and safe padding must be identical across the whole sheet—no packed layout, staggered parts, variable gutters, or edge-hugging pieces. If the largest piece does not fit, reduce the whole sheet uniformly rather than tightening one cell. Every piece must be complete, fully separated, and must not touch or cross any cell boundary. The weapon must not touch either arm. Use a transparent background, no labels, and no assembled character.`;
  const joints = standardHumanoid
    ? "The pelvis cell must contain only the waist-and-hip piece: it stops at the hip sockets and contains no upper-leg or thigh segment, and it is never a shoulder pad. Each upper-arm cell ends at the elbow and contains no forearm or hand. Each forearm cell runs from elbow to wrist and contains exactly one hand, so the entire sheet shows exactly two hands total and no hand appears in any other cell. Each thigh ends at the knee with no shin or foot; each shin runs from knee to foot. Keep every limb segment oriented top-to-bottom with its proximal joint at the top and a small matching overlap at each joint."
    : "Split articulated limbs at usable joints and leave a small matching overlap at every connection. Keep each limb segment consistently oriented with its proximal joint clearly identifiable. Never place a whole assembled character or multiple unrelated pieces inside one cell.";
  const uniqueness = standardHumanoid
    ? "Do not duplicate, mirror-copy, or reuse one arm or leg as its opposite-side part. Left and right pieces must each be derived once from the corresponding side of the reference character. Before output, verify all 12 cell meanings, exactly two hands, one pelvis, one weapon, no repeated limb, and no whole arm or whole leg."
    : `Do not duplicate, mirror-copy, or reuse one side as the opposite-side component. Before output, verify that all ${count} cells contain exactly one distinct useful component and that no component is missing, repeated, or cut by a cell boundary.`;
  const description = options.description?.trim() ? ` Character description: ${options.description.trim()}.` : "";
  const extra = options.extra?.trim() ? ` Extra requirements: ${options.extra.trim()}.` : "";
