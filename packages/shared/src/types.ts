...
export function buildArticulatedPartsPrompt(options: { description?: string; reference?: boolean; extra?: string; rows?: number; cols?: number }): string {
  ...
  const separation = `Use a regular ${cols} × ${rows} lattice of identical cells; do not draw grid lines. One complete isolated part per cell, opaque bounding box centered, opposite margins balanced, and at least 10% clear padding from every boundary. Use one global scale factor; if needed shrink all parts uniformly. Identical center spacing and row/column pitch; no variable gaps, packed/staggered layout, touching, crossing, labels, or assembled character. Weapon must not touch an arm.`;
