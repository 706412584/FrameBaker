...
import { validateFbanimEntryPath, validateFbanimManifest, validateMotionClip, validateRenderProfile, type CharacterBinding, type Mat4, type MotionClipV2, type RenderProfile } from "../packages/shared/src";
...
describe("RenderProfile 与烘焙纯逻辑", () => {
  ...
  test("MotionClip v2 重复时重建跨循环片段且不产生重复时间", () => {
    const first = { type: "cubic-bezier" as const, x1: .2, y1: .1, x2: .8, y2: .9 };
    const second = { type: "cubic-bezier" as const, x1: .1, y1: .2, x2: .9, y2: .8 };
    const clip: MotionClipV2 = { schemaVersion: 2, kind: "motion-clip", id: "curve", name: "Curve", skeletonId: "skeleton", duration: 1, loop: true, tracks: [{ targetId: "root", property: "translation", keyframes: [{ time: 0, value: [0, 0, 0], outInterpolation: first }, { time: .5, value: [1, 0, 0], outInterpolation: second }, { time: 1, value: [9, 0, 0], outInterpolation: null }] }], events: [] };
    const configured = configuredMotionClipForRaster(clip, 1, 2) as MotionClipV2;
    expect(configured.tracks[0]!.keyframes.map((key) => [key.time, key.value[0]])).toEqual([[0, 0], [.5, 1], [1, 0], [1.5, 1], [2, 9]]);
    expect(configured.tracks[0]!.keyframes.map((key) => key.outInterpolation)).toEqual([first, second, first, second, null]);
    expect(validateMotionClip(configured).ok).toBeTrue();
  });
  test("零时长单 key 的 MotionClip v2 重复后仍以 terminal null 收束", () => {
    const clip: MotionClipV2 = { schemaVersion: 2, kind: "motion-clip", id: "zero", name: "Zero", skeletonId: "skeleton", duration: 0, loop: false, tracks: [{ targetId: "root", property: "scale", keyframes: [{ time: 0, value: [1, 1, 1], outInterpolation: null }] }], events: [] };
    const configured = configuredMotionClipForRaster(clip, 1, 3) as MotionClipV2;
    expect(configured.tracks[0]!.keyframes).toEqual([{ time: 0, value: [1, 1, 1], outInterpolation: null }]);
    expect(validateMotionClip(configured).ok).toBeTrue();
  });
