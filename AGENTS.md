...
- Motion clips support `att:<attachmentId>` attachment offset tracks (translation/rotation/scale/deform; deform carries the bend delta in value.x, att:-only): sampled values land in `EvaluatedPose.attachmentOffsets`, and the rendered part matrix is `boneWorld × rest × offsetMatrix(t)` — see docs/pose-motion-system.md §15.
