...
export function register(server: McpServer) {
  server.registerTool(
    "list_folders",
    {
      title: "List Folders",
      description:
        "List all folders of a given kind (material or project), returned as a flat list. Frontend groups them into a tree by parent_id.",
      inputSchema: z.object({
        kind: z.enum(["material", "project"]).describe("Folder kind"),
      }),
  ...
  server.registerTool(
    ...
    {
      title: "Create Folder",
      description: "Create a new folder for organizing materials or projects.",
      inputSchema: z.object({
        kind: z.enum(["material", "project"]).describe("Folder kind"),
  ...
  server.registerTool(
    "delete_folder",
    {
      title: "Delete Folder",
      description:
        "Delete a folder and all its descendant folders. Resources (materials/projects) inside the deleted subtree are moved up to the deleted folder's parent (not deleted).",
      inputSchema: z.object({
    ...
    async ({ folderId }) => {
      ...
      if (!row) return err("文件夹不存在");
      const kind = row.kind as FolderKind;
      const table = kind === "material" ? "materials" : "projects";
      ...
      tx();
      broadcast("folders_changed", { kind });
      if (kind === "material") broadcast("materials_changed", {});
      return ok({ ok: true });
    }
  );
  ...
  server.registerTool(
    "move_items_to_folder",
    {
      ...
      description: "Move materials or projects to a folder. Use folderId=null to ungroup (move to root).",
      inputSchema: z.object({
        kind: z.enum(["material", "project"]).describe("Item kind"),
    ...
    async ({ kind, ids, folderId: rawFolderId }) => {
      ...
      if (kind === "material") broadcast("materials_changed", {});
      else broadcast("folders_changed", { kind });
