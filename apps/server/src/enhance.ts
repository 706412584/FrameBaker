...
function buildSystem(style?: string, mediaKind: "image" | "video" = "image", referenceImageCount = 0): string {
...
export async function enhancePrompt(req: EnhancePromptRequest): Promise<EnhancePromptResponse> {
  ...
  const messages: Array<{ role: "system" | "user" | "assistant"; content: string }> = [
    { role: "system", content: buildSystem(req.style, req.mediaKind, req.referenceImageCount) },
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (!invalidEnhancedPrompt(raw)) return { enhanced: singleLine(raw), enhancerName: enhancer.name };
    if (attempt === 0) {
      messages.push(
