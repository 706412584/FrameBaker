// UI 智能切片 — 迁移自 sprite 工坊 uiSmartSlice.ts。
// 算法与 sprite 原版一致（连通域检测复用 ops.ts 的单份实现，见 connectedComponentsOnMask）；
// I/O 边界改为纯像素输入（Uint8ClampedArray），供 worker / 主线程 / 单测共用。
import { connectedComponentsOnMask, type CropRect } from "../imageops/ops";
...
/** 纯像素版 analyzeUiSmartSlices：行为与 sprite analyzeUiSmartSlices 逐位一致（同 options、同输入） */
export function analyzeUiSmartSlicesData(
  data: Uint8ClampedArray,
  width: number,
  height: number,
  inputOptions: Partial<UiSmartSliceOptions> = {}
): UiSmartSliceResult {
  const options = { ...defaultUiSmartSliceOptions, ...inputOptions };
  const hasTransparency = detectTransparency(data, options.alphaThreshold);
  ...
  const contentMask = buildContentMask(data, width, height, options.alphaThreshold, options.colorThreshold);
  const floodMask = hasTransparency
    ? buildContentMask(data, width, height, options.alphaFloodThreshold, options.colorThreshold)
    : contentMask;
  const components = connectedComponentsOnMask(floodMask, width, height);
