...
function parseDragIds(e: React.DragEvent): string[] {
  try {
    const raw = e.dataTransfer.getData("application/x-framebaker-ids");
...
export default function FolderTree({
  ...
  const onDropRow = async (e: React.DragEvent, folderId: string | null) => {
    e.preventDefault();
    setDropTarget(null);
    const folderDrag = e.dataTransfer.getData("application/x-framebaker-folder");
    if (folderDrag) {
    ...
    return (
      <div key={n.id} className="folder-node">
        <div
          ...
          draggable
          onDragStart={(e) => {
            e.dataTransfer.setData("application/x-framebaker-folder", n.id);
