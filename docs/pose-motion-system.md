...
- 固定关节：根/骨盆、胸、颈、头、左右肩肘腕、左右髋膝踝，可选脚尖和武器握持点；
- Pixi 独立骨架编辑画布：拖动关节、复制姿态、左右镜像、恢复默认；
- 动作时间轴：新增/复制/删除/排序关键帧，设置 fps、循环和首尾连续性；
- 关键帧插值：先做局部旋转与根节点位移的线性插值，缓动和 IK 后置；
- 为每帧渲染高分辨率姿态图，也可渲染与目标相同网格的姿态表；
- 内置最小预设：待机、走、跑、攻击、受击、死亡；按 front/back/left/right 区分视角，左右可镜像但前后不可混用；
- 生成时优先一次生成 4–8 帧短精灵表，复杂动作拆成多个 clip。
