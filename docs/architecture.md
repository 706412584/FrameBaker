...
- **Workflow graphs (infinite-canvas node editor)** (`apps/server/src/graph/` + `apps/web/src/graph/` + `packages/shared/src/graph.ts`): a graph is four tables `graphs`/`graph_nodes`/`graph_edges`/`graph_outputs`; node types registered in `graph/registry.ts` (port type validation via `portsCompatible` — mismatched connections rejected, single-input-port enforced by `UNIQUE(to_node,to_port)`). The executor `graph/executor.ts` topologically sorts → runs node by node → **content-addressed caching**: `content_hash = sha256(node_type + canonical_json(params) + upstream port hashes concatenated in port-name order)`, source nodes use material file size+mtime; a `graph_outputs` hit skips the node, artifacts land in `storage/graph/outputs/<hash>/` (idempent overwrite). Unlike the global queue, graph execution payloads persist entirely in `graph_outputs` (queue in-memory payloads are lost on restart; graph's are not) — after restart, completed nodes hit cache directly. Node implementations `graph/nodes.ts` reuse `jobs/*` low-level functions (ffmpeg extraction, `runMatting`), keeping the dependency direction graph/ → jobs/ one-way. Execution state reaches frontend nodes via WS `graph_node_status` (running/done/skipped-cache/error/cancelled), controlled by `POST /api/graphs/:id/run|cancel`, with `/running` polling as fallback. Frontend `apps/web/src/graph/GraphPage.tsx` uses `@xyflow/react` (React Flow): graph list on the left, infinite canvas on the right, node drag positions debounce-persisted at 400ms. Implementation plan and phases in `docs/graph-workflow-plan.zh-CN.md` (zh).
...
- Workflow graph tables: `graphs(id, name, folder_id, created_at, updated_at)`, `graph_nodes(id, graph_id, type, params, x, y)`, `graph_edges(id, graph_id, from_node, from_port, to_node, to_port, UNIQUE(to_node,to_port))`, `graph_outputs(content_hash, port, node_type, payload, created_at, PRIMARY KEY(content_hash,port))` — output cache is keyed by content hash, shared across graphs, deleting a graph doesn't invalidate outputs

## Frontend Pages & Components

- `App.tsx`: `/` project list ↔ `/project/:id` editor ↔ `/materials` material library ↔ `/graphs` workflow canvas ↔ `/settings` settings page (history.pushState + popstate); globally suppresses browser native context menu (preserves input/textarea for paste; frames use custom ContextMenu)
- `TopNav`: primary nav (Projects / Materials / Workflows / Settings) + theme toggle (three-state: follow system / light / dark) + language toggle (zh/en, `LangToggle`); editor page has its own top bar and doesn't show this
- `graph/GraphPage`: workflow page — graph list on the left (create/delete, refreshed by WS `graphs_changed`), React Flow infinite canvas on the right (add nodes via dropdown, drag-to-connect, disconnect, run/cancel buttons, node status outline: running=accent / done=green / cache-hit=cyan / error=red); node positions debounce 400ms PATCH persistence, run progress backfills via WS `graph_node_status`; double-click a node (or right-click → Edit params) opens a params panel (renders number/text/boolean fields from paramsSchema, materialId as a material dropdown, PATCH on save refreshes the node card summary); edges are port-type-colored beziers (shared `PORT_COLORS`: video red / image green / rect cyan / sheet orange / palette yellow, hover thickens, click deletes), with connection anchors being real React Flow `<Handle>`s inside nodes (missing Handles make edges silently not render); right-clicking a node opens a context menu (Edit params / Delete)
