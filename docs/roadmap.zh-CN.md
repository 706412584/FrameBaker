...
- **MCP 服务端**：内置 Model Context Protocol 端点（`POST /mcp`，Streamable HTTP + JSON-RPC 2.0，协议版本 `2024-11-05`），34 个工具覆盖项目/帧/素材/生成/抠图/文件夹/任务/系统配置全功能；工具直接操作 db 与内部模块（零 HTTP 自调用开销）；兼容 Claude Desktop / Cursor / Windsurf 等 AI 客户端
